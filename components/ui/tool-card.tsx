...
export function ToolCard({ id, title, description, icon, colorClass, hoverBorderClass }: ToolCardProps) {
  return (
    <Link href={`/tools/${id}`}>
      <motion.div
        whileHover={{ scale: 1.02 }}
        whileTap={{ scale: 0.98 }}
        className={`p-4 bg-zinc-900/40 border border-zinc-800/50 rounded-xl hover:bg-zinc-800/50 transition-colors cursor-pointer group ${hoverBorderClass} shadow-sm hover:shadow-md h-full`}
      >
